...
class TrainAgent:
    ...
    def __init__(self, cfg):
        ...
        self.eval_n_envs = cfg.get("eval_n_envs", self.n_envs)  # Default to same as training
        
        # Check for run_eval flag to actually create evaluation environments
        if cfg.get('run_eval', False):
            log.info(f"Creating separate evaluation environments with {self.eval_n_envs} envs (run_eval=True)")
            ...
            if not env_type == "furniture":
                self.eval_venv.seed(
                    [self.seed + 10000 + i for i in range(self.eval_n_envs)]
                )  # Different seeds from training envs
            log.info(f"Evaluation environments created successfully")
    ...
    def reset_env_all(self, verbose=False, options_venv=None, eval_flag=False, **kwargs):
        # Choose which environment to reset based on eval_flag
        if eval_flag:
            # Use eval environments
            if not hasattr(self, 'eval_venv'):
                raise ValueError("eval_flag=True but no eval_venv found. Make sure eval environments are created.")
            venv = self.eval_venv
            n_envs = self.eval_n_envs if hasattr(self, 'eval_n_envs') else self.n_envs
        else:
            # Use training environments (default behavior)
            venv = self.venv
            n_envs = self.n_envs
            
        if options_venv is None:
            options_venv = [
                {k: v for k, v in kwargs.items()} for _ in range(n_envs)
            ]
        ...
        if isinstance(obs_venv, list):
            obs_venv = {
                key: np.stack([obs_venv[i][key] for i in range(n_envs)])
                for key in obs_venv[0].keys()
            }
        if verbose:
            env_type = "eval" if eval_flag else "train"
            for index in range(n_envs):
                logging.info(
                    f"<-- Reset {env_type} environment {index} with options {options_venv[index]}"
                )
        return obs_venv
